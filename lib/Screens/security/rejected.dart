            ...
            children: _items.map((MyItem item){
              return ExpansionPanel(
                headerBuilder: (BuildContext context  ,bool isExpanded){
                  return Text(item.header,style: TextStyle(color: Colors.red,fontSize: 20));
                },
                isExpanded:item.isExpanded,
                body: Container(
                    child: Text(item.body,style: TextStyle(fontWeight: FontWeight.bold,color:Colors.red,fontSize: 18),)
                ),
              );
            }).toList(),
          )
